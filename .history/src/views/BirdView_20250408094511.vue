...
export default {
  ...
  setup() {
    ...
    try {
        ...
        console.log(⁠ Sending image ${imageFile.name} to ${apiUrl} ⁠);

        // Send POST request - * Remove the manual headers option *
        ...
        return response.data; // Should contain { detections: [...] }
        ...
        console.error('No response received from server. Check network connection, server status at ' + (error.config?.url || 'API URL') + ', and CORS configuration on the server.');
        } else {
