  ...
  setup() {
    ...
    const fetchRecommendedPlants = async () => {
      ...
      try {
        ...
        console.log('API返回的植物数组:', plantData) // 添加控制台输出
    ...
    const getPlantImage = (plant) => {
      ...
      if (!plant || !plant.plantName) {
        console.log('未找到植物或植物名称，使用默认图片')
        return '/assets/images/plants/default-plant.jpg'
      }

      // 如果有植物名称，直接使用名称作为图片文件名
      if (plant.plantName) {
        // 去除空格，确保首字母大写，其余小写，与图片命名格式匹配
        // 例如："Olive Tree" -> "OliveTree.jpg"
        const formattedName = plant.plantName
          .replace(/\s+/g, '') // 移除所有空格
          .replace(/^\w/, (c) => c.toUpperCase()) // 确保首字母大写

        return `@/assets/images/plants/${formattedName}.jpg`
      }
...
.banner {
  background-image:
    ...
    url('@/assets/images/plants/default-plant.jpg');
