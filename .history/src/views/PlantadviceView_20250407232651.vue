          ...
          <div v-for="(plant, index) in recommendedPlants" :key="index" class="plant-card">
            <div class="plant-image">
              <img
                ...
                :alt="plant.name"
                onerror="this.src='/assets/images/plants/default-plant.jpg'"
              />
            </div>
...
export default {
  ...
  setup() {
    ...
    const fetchRecommendedPlants = async () => {
      ...
      try {
        recommendedPlants.value = await getRecommendedPlants(
          selectedState.value,
    ...
    const getPlantImage = (plant) => {
      ...
      // 如果有指定imageName字段，优先使用它
      if (plant.imageName) {
        return `/assets/images/plants/${plant.imageName}`
      }

      // 如果有name属性，基于名称生成图片路径
      if (plant.name) {
        // 转换为小写，替换空格为下划线
        const imageName = plant.name.toLowerCase().replace(/\s+/g, '_') + '.jpg'
        return `/assets/images/plants/${imageName}`
      }
