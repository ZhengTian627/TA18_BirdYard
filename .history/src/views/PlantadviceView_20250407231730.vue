<template>
  <div class="plant-advice-container">
    ...
    <!-- Main Content Area -->
    <div class="content-section" v-if="selectedState">
...
export default {
  ...
  setup() {
    ...
    const stateInfo = reactive({
      ...
      NSW: {
        climate: 'Diverse climate, mild coastal areas, dry inland',
        ecosystem: 'Coastal forests, Blue Mountains forests, and inland grasslands',
        birdSpecies: 'Australian Wren, Red-tailed Black Cockatoo, Lyrebird',
        soilTypes: 'Acidic soils in coastal areas, alkaline soils inland',
        rainfall: 'Average 900mm in coastal areas, below 400mm inland',
        plantingTips:
          'Choose moisture-tolerant plants for coastal areas, drought-resistant varieties for inland',
      },
    })
    ...
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        const mapboxUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(address.value)}.json?access_token=${mapboxToken}&country=au&limit=5`
        ...
        if (data.features && data.features.length > 0) {
          ...
          try {
            const nominatimUrl = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address.value)}&limit=5&countrycodes=au`
    ...
    const selectSuggestion = async (index) => {
      ...
      } else if (addressLower.includes('new south wales') || addressLower.includes('nsw')) {
        selectedState.value = 'NSW'
      } else {
        ...
        try {
          ...
          if (data.features && data.features.length > 0) {
            ...
            if (
              ...
              selectedState.value = stateName === 'New South Wales' ? 'NSW' : stateName
            } else {
              locationError.value =
                'We currently only service Queensland, Victoria, and NSW. Please select a location in these states.'
              selectedState.value = ''
            }
          } else {
            locationError.value =
    ...
    const fetchRecommendedPlants = async () => {
      ...
      try {
        ...
        recommendedPlants.value = response.data
      } catch (err) {
