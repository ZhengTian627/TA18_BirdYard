  ...
  setup() {
    ...
    const getPlantImage = (plant) => {
      if (!plant || !plant.plantName) {
        return '/img/plants/default-plant.jpg' // 注意这里改为public路径
      }
      ...
      // 如果您使用了Vite作为构建工具，尝试这种方式
      try {
        // 这种方法可能适用于Vite项目
        return new URL(`/src/assets/images/plants/${formattedName}.jpg`, import.meta.url).href
      } catch (e) {
        // 备选方案 - 如果在生产环境中运行，使用不同的路径
        return process.env.NODE_ENV === 'production'
          ? `/assets/images/plants/${formattedName}.jpg`
          : `/src/assets/images/plants/${formattedName}.jpg`
      }
    }
